...
const vec4 outOfBounds = vec4(0.0, 0.0, 0.0, 1.0);
...
void main() {
	...
	gl_FragColor = video;
	//gl_FragColor.a = alphatest.a;
	gl_FragColor.rgb *= alphatest.a;

	
	float apply = abs(sin(curUV.y*720.0*0.5)*0.5);
	gl_FragColor.rgb = mix(gl_FragColor.rgb, vec3(0.0), apply);
	
	if(curUV.x<0.0 || curUV.x>1.0 || curUV.y<0.0 || curUV.y>1.0){
		gl_FragColor = outOfBounds;
	}
