...
float NebulaGenerator(vec2 uv){
 ...
 vec3 NebulaLayer(vec2 uv, float i){
	...
	vec2 r = vec2(i,fract(i*22.1)) * 232.;
...
void main() {
	 if(aaaaaa > 0.01){
		...
		vec3 col = vec3(0.);
		float backNoise = NebulaGenerator(uv*10.0);
		col = vec3(backNoise*0.09,backNoise*0.07,backNoise*0.035);

		for(float i=0.;i<1.;i+=1./NUM_LAYERS){
			vec2 movingUV = vec2(uv.x,uv.y-((iTime+timAdd)*ySpeed*(i+0.5)));
			float depth = i;
			float scale = mix(ZOOM, .5, depth);
			float fade = depth * smoothstep(1.,.9,depth);
			if(generateNebula){col += NebulaLayer(movingUV*scale+(i*453.2), i)*fade*1.5;}
			col += StarLayer(movingUV*scale+(i*453.2))*fade*.75;
		}
		col *= vec3(aaaaaa);
